...
#source "/vendor/github.com/rcaloras/bash-preexec/bash-preexec.sh"
...
#_library_finalize_hook+=('__bp_install_after_session_init')

## Helper functions
function __check_precmd_conflict() {
	local f
	__bp_trim_whitespace f "${1?}"
	_array-contains-element "${f}" "${precmd_functions[@]}"
}

function __check_preexec_conflict() {
	local f
	__bp_trim_whitespace f "${1?}"
	_array-contains-element "${f}" "${preexec_functions[@]}"
}
